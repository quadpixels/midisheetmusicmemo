...
public class MidiPlayer extends LinearLayout {
    ...
    private int playstate;               /** The playing state of the Midi Player */
    ...
    public void SetMidiFile(MidiFile file, MidiOptions opt, SheetMusic s) {
         ...
         * highlighted notes.
         */
        if ((file == midifile && midifile != null && getPlaystate() == paused)) {
            options = opt;
            sheet = s;
    ...
    Runnable ReShade = new Runnable() {
      public void run() {
        if (getPlaystate() == paused || getPlaystate() == stopped) {
    ...
    public void Play() {
        ...
        else if (getPlaystate() == initStop || getPlaystate() == initPause || getPlaystate() == playing) {
            return;
        }
    ...
    public void Pause() {
        ...
        else if (getPlaystate() == playing) {
            setPlaystate(initPause);
            return;
        }
    }
    ...
    public void Stop() {
        this.setVisibility(View.VISIBLE);
        if (midifile == null || sheet == null || getPlaystate() == stopped) {
            return;
        }

        if (getPlaystate() == initPause || getPlaystate() == initStop || getPlaystate() == playing) {
            /* Wait for timer to finish */
            setPlaystate(initStop);
            DoStop();
        }
        else if (getPlaystate() == paused) {
            DoStop();
     ...
     */
    void Rewind() {
        if (midifile == null || sheet == null || getPlaystate() != paused) {
            return;
        }
    ...
    Runnable TimerCallback = new Runnable() {
      public void run() {
        ...
        else if (getPlaystate() == stopped || getPlaystate() == paused) {
            /* This case should never happen */
            return;
        }
        else if (getPlaystate() == initStop) {
            return;
        }
        else if (getPlaystate() == playing) {
        ...
        else if (getPlaystate() == initPause) {
            long msec = SystemClock.uptimeMillis() - startTime;
            StopSound();
